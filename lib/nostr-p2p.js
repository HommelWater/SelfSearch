...
export class NostrP2P {
    constructor(secretKeyHex, options = {}) {
        ...
        // Reconnect relay sockets on drop instead of giving up; log failures so
        // relay outages are visible in the console instead of silent. Tests can
        // swap in a fake pool via NostrP2P.SimplePool (no network, no module
        // mocking).
        this.pool = new (NostrP2P.SimplePool || SimplePool)({
            enableReconnect: true,
            onRelayConnectionFailure: (url) => log(`[p2p] relay unreachable: ${url}`)
        });
        ...
        // When true, allow connecting to our own npub (multi-device sync where
        // several devices share one identity key). Own published signals are
        // still skipped so a device never handshakes with itself.
        this.allowSelf = options.allowSelf || false;
        this._ownSignals = new Set();
        ...
        // Recover promptly when the page/network wakes back up instead of
    ...
    // Relay socket status: Map(relayUrl -> connected). Empty map = no relay has
    // been reachable yet.
    relayStatus() {
        try { return this.pool.listConnectionStatus(); } catch { return new Map(); }
    }

    // Initiate a connection to a peer. With `allowSelf` this includes our own
    // npub — used to link devices that share one identity key.
    connect(npub) {
        if ((!this.allowSelf && npub === this.npub) || this.sessions.has(npub)) return;
        if (this.sessions.size >= this.maxConnections + this.pendingSlots) return;
        let peerPk;
        try { peerPk = nip19.decode(npub).data; } catch { return; } // bad npub: skip, don't kill the reconnect pass
        this._createSession(npub, true, peerPk);
    }

    addPeer(npub) {
        if (this.peers.has(npub)) return;
        // With allowSelf (multi-device sync) our own npub stands in for the
        // other devices sharing our identity key — we must "know" it to accept
        // their inbound handshake and to keep reconnecting to it.
        if (npub === this.npub && !this.allowSelf) return;
        this.peers.add(npub);
        this._maintenance();
    }
    ...
    _createSession(npub, initiator, peerPk) {
        ...
        pc.oniceconnectionstatechange = () => {
            // ICE 'failed' means no viable pair was found — recovering on the
            // same pc is unlikely. Drop so the maintenance loop rebuilds the
            // connection (with fresh ICE) instead of waiting out the silence
            // timeout. 'disconnected' is transient and left to liveness.
            if (pc.iceConnectionState === 'failed' && this.sessions.get(npub) === session) {
                this._dropSession(npub, 'ice failed');
            }
        };
        ...
        if (initiator) {
            ...
            pc.createOffer()
                ...
                .then(() => {
                    ...
                    session.offerEventId = this._sendSignal(session.peerPk, { type: 'offer', ots: session.offerTs, sdp: { type: pc.localDescription.type, sdp: pc.localDescription.sdp } });
                })
    ...
    _maintenance() {
        ...
        // Expire offer dedup entries so a genuinely new handshake from a peer
        // we dropped is never blocked by an old timestamp.
        for (const [npub, ts] of Object.entries(this._lastOfferAt)) {
            if (now - ts > PENDING_TIMEOUT) delete this._lastOfferAt[npub];
        }
        ...
        for (const [npub, s] of Array.from(this.sessions)) {
            if (s.phase === 'connecting') {
                ...
                } else if (!s.initiator && s.pc.localDescription) {
                    // Answering side: our answer *and* candidates may have been
                    // lost before the offerer's session existed — re-send both.
                    // (Without this, a lost answer sat until the handshake
                    // timed out and the peer re-offered, delaying reconnects.)
                    try {
                        this._sendSignal(s.peerPk, { type: 'answer', sdp: s.answerSdp || { type: s.pc.localDescription.type, sdp: s.pc.localDescription.sdp } });
                        if (s.myCandidates.length) this._sendSignal(s.peerPk, { type: 'ice-candidates', candidates: s.myCandidates });
                    } catch { /* retry next tick */ }
                }
                continue;
        ...
        const candidates = Array.from(this.peers).filter(p => {
            if (this.sessions.has(p)) return false;
            // Device link: with allowSelf, re-connect to our own npub (the
            // other devices) whenever the session is gone instead of waiting
            // for an inbound offer.
            if (p === this.npub) return this.allowSelf;
            return true;
        });
    ...
    _sendSignal(recipientPk, payload) {
        ...
        // Fire-and-forget. Never await relay publishes from inside the signal
        // handler: a slow/unresponsive relay would otherwise stall the whole
        // per-peer signal queue, wedging every reconnect to that peer forever.
        Promise.allSettled(this.pool.publish(this.relays, event)).catch(() => {});
        // Remember our own event ids so a device never handshakes with itself
        // (relays echo our own publishes back into our subscriptions).
        if (this._ownSignals.size > 300) this._ownSignals.clear();
        this._ownSignals.add(event.id);
        return event.id;
    }

    // Serialize processing per peer: concurrent offer/answer handling races
    // corrupt RTCPeerConnection state. Each handler is time-capped so a stuck
    // one can never wedge the queue (and thus all reconnects) for this peer.
    handleSignal(event) {
        // Never handshake with ourselves (same identity in a second tab, or a
        // relay echo of our own publish). With `allowSelf` we still accept
        // signals from OTHER devices sharing our key, but never our own events.
        if (event.pubkey === this.pk && (!this.allowSelf || this._ownSignals.has(event.id))) return;
        const key = event.pubkey;
        const prev = this._signalQueues.get(key) || Promise.resolve();
        const next = prev
            .then(() => Promise.race([
                this._handleSignal(event),
                new Promise((_, reject) => setTimeout(() => reject(new Error('signal handling timed out')), 10000))
            ]))
            .catch((e) => { log(`[p2p] signal error from ${key.slice(0, 12)}: ${e?.message || e}`); });
    ...
    async _handleSignal(event) {
        ...
        if (payload.type === 'offer') {
            ...
            if (this._lastOfferAt[npub] && offerTs <= this._lastOfferAt[npub] && session) {
                // Duplicate/re-sent offer of a handshake we're still working
                // on: if we're answering it, our answer/candidates may have
                // been lost – send them again. If we have *no* session for
                // this peer, fall through and treat it as a fresh handshake:
                // a peer that dropped us and re-offers within the same second
                // as its previous offer was otherwise discarded forever.
                // (Checked *before* the age guard: the initiator re-sends the
                // same offer until the handshake lands, and re-answering an
                // in-flight handshake must not depend on how old its ots got.)
                if (!session.initiator && session.phase === 'connecting' && session.pc.localDescription) {
                    try {
                        this._sendSignal(senderPk, { type: 'answer', sdp: session.answerSdp || { type: session.pc.localDescription.type, sdp: session.pc.localDescription.sdp } });
                        if (session.myCandidates.length) this._sendSignal(senderPk, { type: 'ice-candidates', candidates: session.myCandidates });
                    } catch { /* ignore */ }
                }
                return;
            }
            // Too old to be answerable: the offerer's handshake attempt timed
            // out. Only ignore when it can't be part of a handshake we're still
            // working on — a healthy connected session must not be disturbed,
            // and a session-less old offer just gets a fresh one shortly.
            if (Date.now() - offerTs > PENDING_TIMEOUT && (!session || session.phase === 'connected')) {
                log(`[p2p] ignore old offer from ${npub.slice(0, 12)}`);
                return;
            }
            ...
            if (session) {
                if (session.phase === 'connected') {
                    ...
                    // With the same npub (multi-device sync) the offer event id
                    // breaks the tie deterministically.
                    if (this.npub === npub) {
                        if (session.offerEventId && session.offerEventId > event.id) { log(`[p2p] device glare: keep our offer`); return; }
                        log(`[p2p] device glare: yield to ${npub.slice(0, 12)}`);
                        this._closeSession(npub);
                        session = null;
                    } else if (this.npub > npub) {
                        log(`[p2p] glare: keep our offer to ${npub.slice(0, 12)}`);
                        return;
                    } else {
        ...
        try {
            if (payload.type === 'offer') {
                ...
                session.answerSdp = { type: pc.localDescription.type, sdp: pc.localDescription.sdp };
                this._sendSignal(senderPk, { type: 'answer', sdp: session.answerSdp });
...
// Default relay pool; tests may reassign to a fake to avoid real network I/O.
NostrP2P.SimplePool = SimplePool;
